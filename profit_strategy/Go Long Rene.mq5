...
input double              InpRiskPercent       = 5.0;                // Risk Percentage of Balance
...
void OnTick() {
    ...
    MqlDateTime obsDt = dt; obsDt.hour = InpObserveStartHour;  obsDt.min = InpObserveStartMin;   obsDt.sec = 0;
    MqlDateTime trdDt = dt; trdDt.hour = InpTradingStartHour;  trdDt.min = InpTradingStartMinute;trdDt.sec = 0;
    MqlDateTime clsDt = dt; clsDt.hour = InpClosePositionHour; clsDt.min = InpClosePositionMin;  clsDt.sec = 0;

    datetime obsTime = StructToTime(obsDt);
    datetime trdTime = StructToTime(trdDt);
    datetime clsTime = StructToTime(clsDt);

    // 3. Đóng lệnh cuối ngày
    if(currentTime >= clsTime) {
    ...
    if(currentTime >= obsTime && currentBarTime != lastBarTime) {
        double highM1[];
    ...
    // 5. Kích hoạt lệnh
    if(currentTime >= trdTime && !tradeExecutedToday && sessionHigh > 0) {
        double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
        
        // Điều kiện: Nếu có bật Wait For New Day High thì giá phải vượt sessionHigh
        bool trigger = InpWaitNewDayHigh ? (ask > sessionHigh) : true;
